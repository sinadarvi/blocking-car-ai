...
class Astar(internal var start: state, internal var end: state) {
    private var path = ArrayList<String>()
    private var time1: Long = 0
    private var time2: Long = 0
    private var VisitedList = ArrayList<state>()
    ...
    fun start() {
        ...
        val fringe = PriorityQueue(10000, Comparator<state> { n1, n2 ->
            if (n1.h + n1.CostFromRoot > n2.h + n2.CostFromRoot) {
                1
            } else if (n1.h + n1.CostFromRoot < n2.h + n2.CostFromRoot) {
                -1
            } else {
                0
            }
