...
(private val curent: state) {
    private val list = ArrayList<state>()
    private val temp: state
    internal lateinit var goal: state
    ...
    fun calculate_H(s: state): Int {
        var h = 0
        for (i in 0 until row) {
            for (j in 0 until col) {
                if (s.array[i][j] != goal.array[i][j])
                    h++
            }
        }
        return h
